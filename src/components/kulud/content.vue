...
export default {
  ...
  data () {
    return {
      ...
      kuud: ['Jaanuar','Veebruar','Märts','Aprill','Mai','Juuni','Juuli','August','September','November','Detsember'],
      query:"",
      hetkeKuu : "",
      hetkeKuuNumber: "",
      sorteeriKategooria: ""
    ...
    created(){
      ...
      this.query = encodeURIComponent(`{"where":{"omanik":${owner}, "kuu": ${currentKuu} }}`);
      ...
      tooteBus.$on('misKuu', (data)=>{
        ...
        this.query = this.sorteeriKategooria.length > 1 ? encodeURIComponent(`{"where":{"omanik":${owner},"kuu": ${data},"kategooria":"${this.sorteeriKategooria}" }}`) : encodeURIComponent(`{"where":{"omanik":${owner}, "kuu": ${data} }}`);
        this.items =[];
        this.getData(this.query, token)
      })
      tooteBus.$on('misKategooria', (data)=>{
        let kuu = this.hetkeKuuNumber.length > 0 ? this.hetkeKuuNumber : new Date().getMonth();

        this.sorteeriKategooria = data;
        this.query = encodeURIComponent(`{"where":{"omanik":${owner},"kuu": ${kuu},"kategooria":"${data}" }}`);
        this.items =[];
        this.getData(this.query, token)
      })
  ...
  methods:{
    ...
    getData(query,token ){
      let vm = this;
      axios.get('http://192.168.0.199:3000/api/tooteds?filter='+query+'&access_token='+token.id).then((response)=>{
